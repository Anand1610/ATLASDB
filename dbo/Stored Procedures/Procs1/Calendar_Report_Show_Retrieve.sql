CREATE PROCEDURE [dbo].[Calendar_Report_Show_Retrieve]      ----[F_Calendar_Report_Show_Retrieve] '','','11/18/2014','11/18/2014'
(
	@DomainID			VARCHAR(50),
	@s_a_assigned_user_id					VARCHAR(100) = '',
...
BEGIN		
   ...
   SELECT 
		...
		CONVERT(NVARCHAR(11),EVT.EVENT_DATE,101) +' '+ ISNULL(CONVERT(NVARCHAR(5),EVT.EVENT_TIME,114),'') [EVENT_DATE], 
		ISNULL(EVT.EVENT_NOTES,'') [EVENT_DESCRIPTION],
		EVT.ASSIGNED_TO[ASSIGNED_TO],
		PROVIDER_NAME[PROVIDER_NAME], 
		ISNULL(VW.InjuredParty_FirstName, N'') + N'  ' + ISNULL(VW.InjuredParty_LastName, N'')[INJUREDPARTY_NAME], 
		Court_Name +'     ['+COURT_VENUE+']'[COURT_NAME],
	...
	WHERE 
		EVT.DomainId = @DomainID AND
		 ...
		 (@s_a_assigned_user_id = '' OR  EVT.Assigned_To = @s_a_assigned_user_id)   AND
