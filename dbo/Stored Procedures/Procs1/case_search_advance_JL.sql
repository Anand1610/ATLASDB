...
CREATE PROCEDURE [dbo].[case_search_advance_JL] -- exec [case_search_advance_JL]  @DomainID='JL'
(
	@DomainID						VARCHAR(50),
	...
	@StatusStart					int =0  ,
	@StatusEnd						int =0  ,
	@TotalBalStart					decimal(19,2) =0,
	@TotalBalEnd					decimal(19,2) =0,
...
BEGIN	
	...
	begin
			...
			FROM tblcase where MainDenialReasonsId is not null and tblcase.DomainId=@DomainID
			UNION all
	...
	begin
	insert #temp(case_id)
	select case_id FROM tblcase where  DomainId = @DomainID
	end
	...
	cas.batchCode,
	DateDiff(dd, ISNULL(cas.Date_Status_Changed, cas.Date_Opened), GETDATE()) AS Status_Age,
	 DateDiff(dd,Date_Opened,GETDATE())[Case Age],
	SUM(ISNULL(tre.Claim_Amount,0.00)) - SUM(ISNULL(tre.Paid_Amount,0.00)) - SUM(ISNULL(tre.WriteOff,0.00)) As Total_Balance
	FROM
		tblCase cas (NOLOCK)
		join #temp tmp on cas.Case_Id=tmp.case_id
