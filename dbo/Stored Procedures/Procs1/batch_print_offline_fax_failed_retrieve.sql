CREATE PROCEDURE [dbo].[batch_print_offline_fax_failed_retrieve]
AS
BEGIN	
	SET NOCOUNT ON;
	IF(ISNULL((SELECT COUNT(*) FROM tbl_batch_print_offline_email_fax_status WHERE ISNULL(IsDeleted,0) = 0 AND ISNULL(in_progress,0) = 1 AND ISNULL(faxStatus,'') = 'Failed' AND ISNULL(ResendCount,0) < 4),0) < 2)
	BEGIN
			 SELECT TOP 1
				pk_batch_print_Id,
				Q.[DomainId],
