CREATE PROCEDURE [dbo].[GETPROVIDERS_SETT_FORBILL] --'CO000000000000000043','OI000000000000000192','01/01/2009','01/01/2011'
	@SZ_COMPANY_ID NVARCHAR(20),
	@SZ_OFFICE_ID NVARCHAR(20),
...
insert into #temp
	...
	WHERE 
		dbo.TBLPROVIDER.PROVIDER_ID = (SELECT PROVIDER_ID FROM GREENBILLSPROVIDERS WHERE SZ_COMPANY_ID = @SZ_COMPANY_ID AND SZ_OFFICE_ID = @SZ_OFFICE_ID)
		AND 
...
SELECT 
	...
	convert(varchar(50),convert(decimal(38,2),((ISNULL(SUM(SETTLEMENT_AMOUNT),0)+ISNULL(SUM(SETTLEMENT_INT),0))*100/(isnull(SUM(CLAIM_AMOUNT-PAID_AMOUNT),0.00))))) + '%' AS 'Percentage'
FROM 
	TBLREPORTSETTLEMENT 
WHERE 
	PROVIDER_ID = (SELECT PROVIDER_ID FROM GREENBILLSPROVIDERS WHERE SZ_COMPANY_ID = @SZ_COMPANY_ID AND SZ_OFFICE_ID = @SZ_OFFICE_ID)
	AND 
	CAST(FLOOR(CAST(SETTLEMENT_DATE AS FLOAT))AS DATETIME) >= Replace(@SZ_START_DATE,'/','-') and  
	CAST(FLOOR(CAST(SETTLEMENT_DATE AS FLOAT))AS DATETIME) <= Replace(@SZ_END_DATE,'/','-')  
	AND 
