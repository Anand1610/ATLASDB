CREATE PROCEDURE [dbo].[FHA_WorkArea_SettlementBillGrid]
...
create table #temp(Settled int, treatment_id int,case_id nvarchar(50),DateOfService_Start datetime,
		...
		DENIALREASONS_TYPE nvarchar(200), F_Fee money, Settlement_Pctg float, AttorneyFee money, SettlementInt float)
...
SERVICE_TYPE,
DENIALREASONS_TYPE, 
	CASE when FilingFeeAmt is null then '0.0'
		else FilingFeeAmt end as F_Fee, 
	...
	CASE when AttorneyFee is null then '0.0' 
		else AttorneyFee end as AttorneyFee, 
	CASE when SettlementInt is null then '0.0'
		else SettlementInt end as SettlementInt from tbltreatment b where case_id = @Case_Id
