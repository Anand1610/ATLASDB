create PROCEDURE [dbo].[F_Case_Date_Update_new_AAIndex](
...
BEGIN   
	...
	BEGIN
		...
		BEGIN
			...
			from  tblcase t1 
			JOIN #TEMP t2  On t1.IndexOrAAA_Number = t2.CaseId '
		...
		begin
			...
			,''Activity'',1,t1.Case_Id,getdate(),'''+@User_Id+''','''+ @DomainId+'''
			from  tblcase t1 
			JOIN #TEMP t2  On t1.IndexOrAAA_Number = t2.CaseId  where t1.DomainId='''+@DomainId+''' '
									
		END
		set @update +='t1.'+@fieldName+'= '''+ @newValue+''' '
		set @update+=' from tblcase t1 join #TEMP t3 on t3.CaseId=t1.IndexOrAAA_Number '
    ...
    BEGIN
		 ...
		 set @InsertNotes+=' 
			...
			join tblcase t2 on  t2.case_id=t1.case_id    join #TEMP t3 on t3.CaseId=t2.IndexOrAAA_Number   where t1.DomainId='''+@DomainId+''' '

			set @update +='t1.'+@fieldName+'= '''+ @newValue+''' '
			set @update+=' from tblCase_Date_Details t1 join tblcase t2 on  t2.case_id=t1.case_id    join #TEMP t3 on t3.CaseId=t2.IndexOrAAA_Number '
			set @update+=' where t1.DomainId='''+@DomainId+''' '
