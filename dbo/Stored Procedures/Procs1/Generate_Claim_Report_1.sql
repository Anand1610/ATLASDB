...
CREATE PROCEDURE [dbo].[Generate_Claim_Report_1] 
...
BEGIN
  ...
  cas.Case_Id AS File_No  ,      
  cas.InjuredParty_LastName + ', ' + cas.InjuredParty_FirstName as Patient,      
  --Provider_Name + ISNULL(' [ ' + pro.Provider_Groupname + ' ]','') as Provider, 
  ISNULL( pro.Provider_Groupname ,'') as Provider,
  ins.InsuranceCompany_Name AS Insurance_Carrier,      
  cas.IndexOrAAA_Number AS AAA_Index_No,      
  convert(decimal(38,2),(convert(money,convert(float,cas.Claim_Amount)))) as Billed_Amount,    
  convert(decimal(38,2),(convert(money,convert(float,cas.Paid_Amount)))) as Paid,    
  convert(decimal(38,2),(convert(money,convert(float,cas.Claim_Amount) - convert(float,cas.Paid_Amount) - ISNULL(cas.WriteOff,0)))) as Claim_Balance_Billed,    
  convert(decimal(38,2),(convert(money,convert(float,cas.Fee_Schedule)))) as [Fee Schedule],    
  convert(decimal(38,2),(convert(money,convert(float,cas.Fee_Schedule) - convert(float,cas.Paid_Amount) - ISNULL(cas.WriteOff,0)))) as Fee_Schedule,     
  cas.Status AS Current_Status,   
   ISNULL(cas.StatusDisposition,'') AS Status_Disposition,    
 
    
  Hearing_Trial_Date  =(select isnull(convert(varchar(20),max(Event_Date),101),'') 
							from  tblevent t
  ...
  convert(decimal(38,2),(select sum(DISTINCT ISNULL(Settlement_Amount,0.00)) FROM tblsettlements  (NOLOCK) WHERE Case_Id = cas.case_id)) as PRINCIPAL_Collected,    
  ...
  convert(decimal(38,2),(select sum(DISTINCT ISNULL(Settlement_Int,0.00)) FROM tblsettlements  (NOLOCK) WHERE Case_Id = cas.case_id)) as INTEREST_Collected,    
  convert(decimal(38,2),(select sum(DISTINCT ISNULL(Settlement_AF,0.00)) FROM tblsettlements  (NOLOCK) WHERE Case_Id = cas.case_id)) as ATTORNEY_FEE_Collected,    
  convert(decimal(38,2),(select sum(DISTINCT ISNULL(Settlement_FF,0.00)) FROM tblsettlements  (NOLOCK) WHERE Case_Id = cas.case_id)) as FILING_FEE_Collected,
  (
					SELECT convert(varchar, ISNULL(MIN(tblTransactions.Transactions_Date),''),101) 
  ...
  Settlement_Date=case when CONVERT(varchar,max(DISTINCT Settlement_Date),101) ='01/01/1900' then '' else CONVERT(varchar,max(DISTINCT Settlement_Date),101) end,
   ISNULL(tblSettlement_Type.Settlement_Type,'') AS Settlemen_Type,
    Voluntary_Payment=(select convert(decimal(38,2),(convert(money,convert(float,sum(ISNULL(transactions_amount,0.00)))))) from tblTransactions (NOLOCK)  where tblTransactions.case_id=cas.Case_Id and DomainId=@DomainId and Transactions_Type in ('PreC','PreCToP')),    
  ...
  Collection_Payment=(select convert(decimal(38,2),(convert(money,convert(float,sum(ISNUll(transactions_amount,0.00)))))) from tblTransactions (NOLOCK) where tblTransactions.case_id=cas.Case_Id and DomainId=@DomainId and Transactions_Type in ('C','I')),    
    --Date_Bill_Sent=convert(varchar, ISNULL(cas.Date_BillSent,''),101)  ,
	Date_Bill_Sent=case when convert(varchar, ISNULL(cas.Date_BillSent,''),101) ='01/01/1900' then '' else convert(varchar, ISNULL(cas.Date_BillSent,''),101) end,
    --Date_Opened=convert(varchar, ISNULL(cas.Date_Opened,''),101), 
	Date_Opened=case when convert(varchar, ISNULL(cas.Date_Opened,''),101) ='01/01/1900' then '' else convert(varchar, ISNULL(cas.Date_Opened,''),101) end,
	--CONVERT(VARCHAR(10),Date_AAA_Arb_Filed,101) AS Dat_Arb_LIT_filed, 
	Dat_Arb_LIT_filed=case when convert(varchar, ISNULL(cas.Date_AAA_Arb_Filed,''),101) ='01/01/1900' then '' else convert(varchar, ISNULL(cas.Date_AAA_Arb_Filed,''),101) end,
	ISNULL(cas.DenialReasons_Type,'') as Denial_Reason,
	ISNULL(dbo.fncGetServiceType(cas.Case_ID),'') AS Service_Type,
	 cas.Date_Status_Changed AS Date_Status_Changed,
  CONVERT(int, CONVERT(VARCHAR,DATEDIFF(dd,cas.date_status_Changed,GETDATE()))) as Status_Age,  
  CONVERT(VARCHAR(10),cas.DateOfService_Start,101) AS Start_DOS,    
  CONVERT(VARCHAR(10),cas.DateOfService_End,101) AS End_DOS,
   CONVERT(VARCHAR(10),Accident_Date,101) AS DOA, 
    ISNULL(cas.Ins_Claim_Number,'') AS Claim_No,
	 ISNULL(Policy_Number,'') AS PolicyNo, 
  Bill_No=(select top 1 ISNULL(bill_number,'') from tblTreatment (NOLOCK) where ISNULL(bill_number,'') <> '' and case_id = cas.case_id and domainid = cas.DomainId),
 ...
 ORDER BY     
  ...
  select * from cte where [Current_Status]<>'IN ARB OR LIT' and PortFolio='RDL-FV4'
  union
  select * from cte where   (PortFolio<>'RDL-FV4'  or PortFolio iS NULL or PortFolio='0')  and [Current_Status]<>'IN ARB OR LIT'
