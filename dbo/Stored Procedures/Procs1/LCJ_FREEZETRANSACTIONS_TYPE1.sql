...
CREATE PROCEDURE [dbo].[LCJ_FREEZETRANSACTIONS_TYPE1](        
...
BEGIN 
		...
		UPDATE TBLTRANSACTIONS SET TRANSACTIONS_STATUS='FREEZED',INVOICE_ID=@IID WHERE PROVIDER_ID=@PROVIDER_ID AND (TRANSACTIONS_STATUS IS NULL or TRANSACTIONS_STATUS = 'CONFIRMED') AND TRANSACTIONS_TYPE IN ('C','I','Prec','PreCToP','PreI','ID') AND (Provider_Id NOT IN (SELECT Provider_Id FROM tblProviderFinancial WHERE DomainId=@DomainId))
		...
		IF @RETURN_FF=1
			UPDATE TBLTRANSACTIONS SET TRANSACTIONS_STATUS='FREEZED',INVOICE_ID=@IID WHERE PROVIDER_ID=@PROVIDER_ID AND (TRANSACTIONS_STATUS IS NULL or TRANSACTIONS_STATUS = 'CONFIRMED') AND TRANSACTIONS_TYPE IN ('FFC') and DomainId=@DomainId
			 ...
			 IF @DomainId='BT'
			UPDATE TBLTRANSACTIONS SET TRANSACTIONS_STATUS='FREEZED',INVOICE_ID=@IID WHERE PROVIDER_ID=@PROVIDER_ID AND (TRANSACTIONS_STATUS IS NULL or TRANSACTIONS_STATUS = 'CONFIRMED') AND TRANSACTIONS_TYPE IN ('RFB','EXP') and DomainId=@DomainId
