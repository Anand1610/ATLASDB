...
CREATE PROCEDURE [dbo].[case_search_advance_optv3] -- [case_search_advance]  @DomainID='amt',@s_a_MainDenialID='2'
(
	@DomainID						VARCHAR(50),
...
BEGIN	
	...
	BEGIN
	SELECT * from (
		SELECT	 TotalRowCount = COUNT(case_id) OVER(),*
		FROM
		(
			SELECT distinct
		--,TotalRowCount = COUNT(cas.case_id) OVER(),
		Dense_rank() OVER (ORDER BY Case_AutoId DESC) AS ROWID,
		cas.Case_AutoId,
		cas.Case_Id,		
	...
	WHERE
		...
		WHERE tbl1.ROWID >= @StartIndex AND tbl1.ROWID <= @EndIndex
