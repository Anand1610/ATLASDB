CREATE PROCEDURE [dbo].[GETPROVIDERS_SUMMONSFILEDDATES] --'0','40522','01/01/2008','01/01/2011','',''
(
...
BEGIN
	...
	SET @STRQUERY = 'SELECT 
				YEAR(DATE_OPENED) AS YEAROPENED,
				COUNT(DISTINCT C.CASE_ID) AS NOOFCASES,
				AVG(DATEDIFF(D,DATE_OPENED,DATE_INDEX_NUMBER_PURCHASED)) AS AVGDAYSINDEX 
			FROM 
			...
			WHERE ISNULL(C.IsDeleted,0) = 0   AND
				DATE_INDEX_NUMBER_PURCHASED IS NOT NULL'
	...
	BEGIN
		SET @STRWHERE  = @STRWHERE +  ' AND INSURANCECOMPANY_GROUPNAME IN (SELECT INSURANCECOMPANY_GROUPNAME FROM TBLINSURANCECOMPANY WHERE INSURANCECOMPANY_ID =  ' + @SZ_INSURANCE_ID  + ') '
	END
	...
	SET @STRGROUP = ' GROUP BY YEAR(DATE_OPENED) ORDER BY YEAR(DATE_OPENED)'
