CREATE PROCEDURE [dbo].[case_search_advance_OptV] -- [case_search_advance]  @DomainID='amt',@s_a_MainDenialID='2'
(
	@DomainID						VARCHAR(50),
...
BEGIN	
	  ...
	  --IF OBJECT_ID('tempdb..#CustSearchTrment') IS NOT NULL
	  --BEGIN
   --   DROP TABLE #CustSearctTrment
	  --END

	  --IF OBJECT_ID('tempdb..#CustSearchTrans') IS NOT NULL
	  --BEGIN
   --   DROP TABLE #CustSearchTrans
	  --END

	  --IF OBJECT_ID('tempdb..#CustSearchCaseDt') IS NOT NULL
	  --BEGIN
   --   DROP TABLE #CustSearchCaseDt
	  --END

	  --IF OBJECT_ID('tempdb..#CustSearchCase') IS NOT NULL
	  --BEGIN
   --   DROP TABLE #CustSearchCase
	  --END

	  --IF OBJECT_ID('tempdb..#CustSearchNts') IS NOT NULL
	  --BEGIN
   --   DROP TABLE #CustSearchNts
	  --END

	    

	
        select 	cas.Case_Id,cas.Case_AutoId, cas.InsuredParty_FirstName,cas.InsuredParty_LastName, cas.InjuredParty_LastName , cas.InjuredParty_FirstName ,  
		cas.Provider_ID,  cas.InsuranceCompany_ID,  cas.IndexOrAAA_Number,cas.Claim_Amount,cas.Paid_Amount,cas.Fee_Schedule,
		cas.WriteOff, cas.Status,  cas.Date_Status_Changed , cas.Initial_Status, cas.Ins_Claim_Number,  cas.Accident_Date, cas.DateOfService_Start ,
		cas.DateOfService_End ,cas.DenialReasons_Type ,cas.StatusDisposition,cas.Rebuttal_Status,cas.Policy_Number,cas.Date_Opened,
		cas.case_code,cas.Assigned_Attorney,cas.Opened_By,cas.Date_AAA_Arb_Filed,cas.batchCode, cas.FK_Packet_ID, cas.Adjuster_Id, cas.PortfolioId, 
		cas.Defendant_Id , cas.Court_Id, cas.Attorney_Id, cas.AttorneyFirmId , cas.location_id, cas.UserId, cas.Date_billsent , cas.MainDenialReasonsId
		INTO #CustSearchCase from tblcase cas with(nolock) 
		...
		Where cas.Domainid=@DomainID AND
		(@s_a_MultipleCase_ID ='' OR cas.Case_Id IN (SELECT CaseId FROM @CaseSearch))
		...
		like '%'+ @s_a_InjuredName + '%'  OR  ISNULL(cas.InjuredParty_LastName,'') + ISNULL(cas.InjuredParty_FirstName,'') like '%'+ @s_a_InjuredName + '%'  OR ISNULL(cas.InjuredParty_LastName,'') like '%'+ @s_a_InjuredName + '%'  OR ISNULL(cas.InjuredParty_FirstName,'') like '%'+ @s_a_InjuredName + '%')
		AND (@s_a_InsuredName ='' OR ISNULL(cas.InsuredParty_FirstName,'')+ISNULL(cas.InsuredParty_LastName,'') 
		like '%'+ @s_a_InsuredName + '%'  OR  ISNULL(cas.InsuredParty_LastName,'') + ISNULL(cas.InsuredParty_FirstName,'') like '%'+ @s_a_InsuredName + '%'  OR ISNULL(cas.InsuredParty_LastName,'') like '%'+ @s_a_InsuredName + '%'  OR ISNULL(cas.InsuredParty_FirstName,'') like '%'+ @s_a_InsuredName + '%')
		...
		AND (@s_a_DOSFrom ='' or (CONVERT(DATETIME,cas.DateOfService_Start) >=CONVERT(DATETIME, CONVERT(VARCHAR, @s_a_DOSFrom,101))  
		and cas.DateOfService_Start IS NOT NULL))
		AND (@s_a_DOSTo ='' or (CONVERT(DATETIME,cas.DateOfService_End) <=CONVERT(DATETIME, CONVERT(VARCHAR, @s_a_DOSTo,101))  
		and cas.DateOfService_End IS NOT NULL))
		AND (@s_a_date_status_changed_from = '' OR (Date_Status_Changed Between convert(datetime,@s_a_date_status_changed_from) 
		And convert(datetime,@s_a_date_opened_To)+1))
	    ...
	    select Case_id, Claim_Amount, Paid_Amount , WriteOff, treatment_id,BILL_NUMBER, SERVICE_TYPE, PeerReviewDoctor_ID, account_number
		INTO #CustSearchTrment
		from tblTreatment with(nolock) Where Domainid=@DomainID OPTION (OPTIMIZE FOR (@DomainID='AF'));

		select Case_id, transactions_amount, Transactions_Type , Transactions_Date, ChequeNo
		INTO #CustSearchTrans
		from tblTransactions with(nolock) Where Domainid=@DomainID OPTION (OPTIMIZE FOR (@DomainID='AF'));

		select case_id, Scheduling_Order_Issue_Date, Facilitation_Summary_Due_Date,Case_Evaluation_Summary_Due_Date, Date_Filed,
		Pretrial_Conf_Date, Plaintiff_Discovery_Responses_Completed_Date, Defense_Answers_to_our_Discovery_Completed_Date
		,Discovery_Stip_Sent_Date, Case_Evaluation_Date, Defense_Discovery_Receipt_Date
		INTO #CustSearchCaseDt
		from tblCase_Date_Details with(nolock) Where Domainid=@DomainID OPTION (OPTIMIZE FOR (@DomainID='AF'));
		
        select Case_Id,Notes_Desc,Notes_Type INTO #CustSearchNts from tblNotes with(nolock) Where Domainid=@DomainID and Notes_Type in ('Delay','Delay Arb')


		Create NonClustered index Index_CaseNCl on #CustSearchCase(Case_id) include (Case_AutoId, InsuredParty_FirstName,InsuredParty_LastName, 
		InjuredParty_LastName , InjuredParty_FirstName ,  
		Provider_ID,  InsuranceCompany_ID,  IndexOrAAA_Number,Claim_Amount,Paid_Amount,Fee_Schedule,
		WriteOff, Status,  Date_Status_Changed , Initial_Status, Ins_Claim_Number,  Accident_Date, DateOfService_Start ,
		DateOfService_End ,DenialReasons_Type ,StatusDisposition,Rebuttal_Status,Policy_Number,Date_Opened,
		case_code,Assigned_Attorney,Opened_By,Date_AAA_Arb_Filed,batchCode, FK_Packet_ID, Adjuster_Id, PortfolioId, 
		Defendant_Id , Court_Id, Attorney_Id, AttorneyFirmId , location_id, UserId, Date_billsent , MainDenialReasonsId)

		Create NonClustered index Index_CaseTrans on #CustSearchTrans(Case_id) include (transactions_amount, Transactions_Type, Transactions_Date, ChequeNo)

		Create NonClustered index Index_CaseTrment on #CustSearchTrment(Case_id) include (Claim_Amount, Paid_Amount , WriteOff, treatment_id,BILL_NUMBER, 
		SERVICE_TYPE, PeerReviewDoctor_ID, account_number)
		
		Create NonClustered index Index_CaseDt on #CustSearchCaseDt(Case_id) include (Scheduling_Order_Issue_Date, Facilitation_Summary_Due_Date,
		Case_Evaluation_Summary_Due_Date, Date_Filed, Pretrial_Conf_Date, Plaintiff_Discovery_Responses_Completed_Date, 
		Defense_Answers_to_our_Discovery_Completed_Date , Discovery_Stip_Sent_Date,Case_Evaluation_Date, Defense_Discovery_Receipt_Date)

		Create NonClustered index Index_CaseNts on #CustSearchNts(Case_id) include (Notes_Desc,Notes_Type)
	    


 
	declare @temp table (case_id varchar(50))
	...
	begin
			...
			FROM #CustSearchCase  with(NOLOCK) where MainDenialReasonsId is not null 
			UNION all
	...
	FROM	(
		...
		(select top 1 Notes_Desc FROM #CustSearchNts (NOLOCK) WHERE Case_Id = cas.case_id and Notes_Type = 'Delay') as Delay_Notes,
		(select top 1 Notes_Desc FROM #CustSearchNts (NOLOCK) WHERE Case_Id = cas.case_id and Notes_Type = 'Delay Arb') as Delay_ARB_NOTES,
		PacketID=p.PacketID,
		Rebuttal_Status,
		Policy_Number,
		Voluntary_Payment=(select convert(decimal(38,2),(convert(money,convert(float,sum(transactions_amount))))) from #CustSearchTrans (NOLOCK)  where #CustSearchTrans.case_id=cas.Case_Id and  Transactions_Type in ('PreC','PreCToP')),
		Collection_Payment=(select convert(decimal(38,2),(convert(money,convert(float,sum(transactions_amount))))) from #CustSearchTrans (NOLOCK) where #CustSearchTrans.case_id=cas.Case_Id and  Transactions_Type in ('C','I')),
		bill_number=(select top 1 bill_number from #CustSearchTrment (NOLOCK) where ISNULL(bill_number,'') <> '' and case_id = cas.case_id ),
		Date_Opened=convert(varchar, ISNULL(cas.Date_Opened,''),101),
		Similar_To_Case_ID=(Select top 1 a.Case_Id FROM  #CustSearchCase a (NOLOCK) WHERE a.Provider_Id =cas.Provider_Id  and a.InjuredParty_LastName =cas.InjuredParty_LastName    
		...
		Event_Date= (SELECT convert(varchar(10), max(E.Event_Date),101) 
					FROM	tblEvent E
					WHERE	E.DomainId= @Domainid and cas.Case_Id = E.Case_Id
					) ,
	...
	FROM
		#CustSearchCase cas (NOLOCK)
		join @temp tmp on cas.Case_Id=tmp.case_id
		INNER JOIN tblprovider pro (NOLOCK) on cas.provider_id=pro.provider_id 
		INNER JOIN tblinsurancecompany ins (NOLOCK) on cas.insurancecompany_id=ins.insurancecompany_id
		INNER JOIN dbo.tblStatus sta (NOLOCK) on cas.Status=sta.Status_Type AND @DomainId= sta.DomainId
		LEFT OUTER JOIN #CustSearchTrment (NOLOCK) tre on tre.Case_Id= cas.Case_Id
		...
		LEFT OUTER JOIN tblSettlements sett  WITH (NOLOCK) ON sett.DomainId= @Domainid and cas.Case_Id = sett.Case_Id
		LEFT JOIN tblSettlement_Type WITH (NOLOCK) on sett.Settled_Type = tblSettlement_Type.SettlementType_Id
		LEFT JOIN tblAdjusters adj WITH (NOLOCK) ON adj.Adjuster_Id = cas.Adjuster_Id AND adj.DomainId=@DomainID
		LEFT JOIN #CustSearchTrans T WITH (NOLOCK) ON T.Case_Id = cas.Case_Id 
		...
		LEFT JOIN #CustSearchCaseDt Casdet(NOLOCK) ON cas.Case_Id=casdet.case_id
	...
	WHERE
		...
		AND (@s_a_checknumber = '' OR cas.case_id IN (SELECT DISTINCT CASE_ID FROM #CustSearchTrans (NOLOCK)  
		 where REPLACE(ChequeNo,'-','') LIKE '%' + REPLACE(@s_a_checknumber,'-','') + '%'))  
	...
	FROM
		...
		INNER JOIN tblprovider pro (NOLOCK) on cas.provider_id=pro.provider_id 
		INNER JOIN tblinsurancecompany ins (NOLOCK) on cas.insurancecompany_id=ins.insurancecompany_id
		INNER JOIN dbo.tblStatus sta (NOLOCK) on cas.Status=sta.Status_Type AND sta.DomainId=@DomainID
		LEFT OUTER JOIN #CustSearchTrment (NOLOCK) tre on tre.Case_Id= cas.Case_Id
		LEFT OUTER JOIN TXN_CASE_PEER_REVIEW_DOCTOR (NOLOCK) prdoc on prdoc.TREATMENT_ID = tre.treatment_id
		LEFT OUTER JOIN TXN_tblTreatment t_tre (NOLOCK) on t_tre.Treatment_Id = tre.treatment_id
		LEFT JOIN tblPacket p (NOLOCK) on cas.FK_Packet_ID = p.Packet_Auto_ID
		...
		LEFT JOIN tblAdjusters adj (NOLOCK) ON adj.Adjuster_Id = cas.Adjuster_Id AND adj.DomainId=@Domainid
		LEFT JOIN #CustSearchTrans T (NOLOCK) ON T.Case_Id = cas.Case_Id  
		LEFT OUTER JOIN dbo.MST_PacketCaseType pkt_typ  WITH (NOLOCK) ON p.FK_CaseType_Id = pkt_typ.PK_CaseType_Id 
		--LEFT OUTER JOIN tblEvent E  WITH (NOLOCK) ON E.DomainId= cas.Domainid and cas.Case_Id = E.Case_Id
		LEFT OUTER JOIN tbl_portfolio PF (NOLOCK)on cas.PortfolioId=PF.id  
		LEFT JOIN tblDefendant d(NOLOCK) ON d.Defendant_id = cas.Defendant_Id
		...
		LEFT JOIN tblCourt court(nolock) ON cas.Court_id=court.court_id
	WHERE
			(@s_a_MultipleCase_ID ='' OR --cas.Case_Id IN (SELECT s FROM dbo.SplitString(@s_a_MultipleCase_ID,',')))
		...
		AND (@s_a_accidentdate = '' OR CONVERT(VARCHAR,Accident_Date,101) = @s_a_AccidentDate)
		AND (@s_a_accountno = '' OR tre.account_number like '%' + @s_a_AccountNo +'%' OR tre.bill_number like '%' + @s_a_AccountNo +'%')  
		AND (@s_a_checknumber = '' OR cas.case_id IN (SELECT DISTINCT CASE_ID FROM #CustSearchTrans (NOLOCK) WHERE REPLACE(ChequeNo,'-','') 
		LIKE '%' + REPLACE(@s_a_checknumber,'-','') + '%'))  
		AND (@s_a_paymentdatefrom = '' OR CONVERT(VARCHAR,T.Transactions_Date,101) >= @s_a_paymentdatefrom)
		AND (@s_a_paymentdateto = '' OR CONVERT(VARCHAR,T.Transactions_Date,101) <= @s_a_paymentdateto)
		AND (@s_a_filledunfiled = '' OR sta.Filed_Unfiled IN (SELECT items FROM dbo.STRING_SPLIT(@s_a_filledunfiled,','))) 
		AND (@s_a_specialityid = '' OR tre.SERVICE_TYPE IN (SELECT items FROM dbo.STRING_SPLIT(@s_a_specialityid,',')))
