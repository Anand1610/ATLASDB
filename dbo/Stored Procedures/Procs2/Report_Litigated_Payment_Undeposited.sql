CREATE PROCEDURE [dbo].[Report_Litigated_Payment_Undeposited]
...
BEGIN
	...
	,CONVERT(VARCHAR(10),CONVERT(DATETIME,cpt.LIT_Cleared_Date), 120) as Cleared_Date
	,cas.Initial_Status
	,iif(cpt.LIT_Cleared_Date is null,'Open','Cleared') as PayStatus
	...
	LEFT OUTER JOIN tbl_Portfolio pf (NOLOCK) on pf.Id = cas.PortfolioId
	 LEFT OUTER JOIN Assigned_Attorney (NOLOCK) ON cas.Assigned_Attorney = Assigned_Attorney.PK_Assigned_Attorney_ID
	where
	...
	AND (
		  (@dt_a_invFromDate='' AND @dt_a_invToDate='' AND cpt.LIT_PAY_Freeze_Date IS NULL)
		OR
	...
	AND (
		...
		OR
	     (@dt_a_FromclearDate<>'' AND @dt_a_ToclearDate<>'' AND cpt.LIT_Cleared_Date BETWEEN CONVERT(datetime,@dt_a_FromclearDate) AND CONVERT(datetime,@dt_a_ToclearDate))
		)	
	AND cas. Initial_Status='Undeposited'
