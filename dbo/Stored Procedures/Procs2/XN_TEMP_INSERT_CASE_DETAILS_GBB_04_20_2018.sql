...
CREATE PROCEDURE [dbo].[XN_TEMP_INSERT_CASE_DETAILS_GBB_04_20_2018]
...
BEGIN
	...
	DECLARE @IndexOrAAA_Number VARCHAR(100)
	DECLARE @Settled_Awarded VARCHAR(100)
	DECLARE @HEARING_DATE_TIME VARCHAR(250)
	DECLARE @Attorney_Fee VARCHAR(100)
	DECLARE @Filing_Fee VARCHAR(100)
	DECLARE @Date_Closed VARCHAR(100)
	DECLARE @STATUS VARCHAR(250)
	DECLARE @old_Case_id VARCHAR(250)
		
	----- set max id here and change table name-----
	SET @autoidtblcase = 17751 --(select max(case_autoid) from tblcase Where  [DomainId] = 'DL') -- 17751
	Set @s_a_DomainId = 'DL'

	DECLARE CUR_GBB_CASES CURSOR READ_ONLY FORWARD_ONLY LOCAL FOR
	
	SELECT  AUTOID, SZ_CASE_ID, SZ_CASE_NO, SZ_PATIENT_FIRST_NAME,SZ_PATIENT_LAST_NAME, SZ_PATIENT_ADDRESS, SZ_PATIENT_CITY, SZ_PATIENT_STATE, SZ_PATIENT_ZIP, SZ_PATIENT_PHONE, DT_DATE_OF_ACCIDENT, DT_DOB, SZ_POLICY_HOLDER, SZ_POLICY_NUMBER, SZ_CLAIM_NUMBER, SZ_BILL_NUMBER, DT_FIRST_VISIT_DATE, dt_last_visit_date, FLT_BILL_AMOUNT, FLT_PAID, InsuranceCompanyId, Provider_Id,SZ_COMPANY_ID,Denial_Reason1,Denial_Reason2,Denial_Reason3,SZ_CASE_TYPE_NAME,BILL_DATE,DomainId,IndexOrAAA_Number,Settled_Awarded,HEARING_DATE_TIME,Attorney_Fee,Filing_Fee,Date_Closed,STATUS,old_Case_id
	FROM [XN_TEMP_TABLE_GBB_DATA_GoGYB_ALL] where ISNULL(insurancecompanyid ,'') <> ''  and ISNULL(Status_Done,'') = ''  and ISNULL(provider_id,'') <> ''
	...
	BEGIN
		...
		print @Case_Id
			...
			(  
				...
				@SZ_POLICY_NUMBER,  
				@SZ_CLAIM_NUMBER, 
				@STATUS,--'AAA - NEW CASE ENTERED',--'AAA OPEN',  
				GETDATE(),
				'ARB', --'ARB',
				@SZ_CASE_ID,
				@SZ_CASE_NO,
	  ...
	  UPDATE [XN_TEMP_TABLE_GBB_DATA_GoGYB_ALL]
	  SET Status_Done= @Case_Id
	  WHERE AUTOID = @AUTOID
