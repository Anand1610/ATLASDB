...
CREATE PROCEDURE  sp_open_cases_billing_report_JL
    
As    
BEGIN    
--exec sp_open_cases_billing_report_JL    
...
SELECT DISTINCT     
 ...
 ,(select top 1 nt.[Notes_Date] from [dbo].[tblNotes] nt Where nt.[Case_Id] = tblcase.case_id And (nt.[Notes_Desc] Like '%to AAA - FILED%' or nt.[Notes_Desc] Like '%to S & C GENERATED')And year(nt.[Notes_Date])= @year AND month(nt.[Notes_Date])= @month) AS [Case_FILED_Date]    
