CREATE PROCEDURE [dbo].[XN_TEMP_INSERT_CASE_DETAILS_GBB_old]
...
BEGIN
	...
	FROM [XN _TABLE_GBB_DATA_GoGYB_ALL] where ISNULL(insurancecompanyid ,'') <> ''  and ISNULL(Status_Done,'') = ''  and ISNULL(provider_id,'') <> ''
	...
	BEGIN
		...
		BEGIN     
			...
			SET @Case_Id  = 'DK' + RIGHT(CAST(DATEPART(year, GETDATE()) AS NVARCHAR),2) + '-' + CAST(@Case_Id_IDENTITY AS NVARCHAR)  
			UPDATE tblCase SET Case_Id = @Case_Id where Case_AutoId = @Case_Id_IDENTITY
			exec LCJ_AddNotes @Case_Id = @Case_Id,@Notes_Type='Activity',@NDesc='Case Opened',@User_Id='System',@ApplyToGroup=0
		...
		BEGIN
			...
			exec LCJ_AddNotes @case_id=@Case_Id,@notes_type='PROVIDER',@Ndesc='cannot pursue a non no-fault case.',@User_id='system',@Applytogroup=0
		END

	  UPDATE [XN _TABLE_GBB_DATA_GoGYB_ALL]
