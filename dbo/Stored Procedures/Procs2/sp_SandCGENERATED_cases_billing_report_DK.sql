CREATE PROCEDURE sp_SandCGENERATED_cases_billing_report_DK
...
SELECT DISTINCT 
	...
	,(select top 1 nt.[Notes_Date] from [dbo].[tblNotes] nt Where nt.[Case_Id] = tblcase.case_id And nt.[Notes_Desc] Like '%to S & C GENERATED%' And year(nt.[Notes_Date])= @year AND month(nt.[Notes_Date])= @month) AS [Case_FILED_Date]
	...
	WHERE  n.[Notes_Desc] Like '%to S & C GENERATED%' And 
