...
CREATE PROCEDURE [dbo].[Report_Purchase_Test] 
...
--Query to insert into temp table Ends here
	 ...
	 SELECT distinct
		...
		(SELECT VPay FROM #TempVol tep (NOLOCK) WHERE tep.VCaseId=cas.Case_Id AND RowNum=1 AND tep.CPT_ATUO_ID = cod.CPT_ATUO_ID) as TT_PreC_1,
		(SELECT convert(varchar(10), VDate,120) FROM #TempVol tep (NOLOCK) WHERE tep.VCaseId=cas.Case_Id AND RowNum=1 AND tep.CPT_ATUO_ID = cod.CPT_ATUO_ID) as TD_PreC_1,
		(SELECT VPay FROM #TempVol tep (NOLOCK) WHERE tep.VCaseId=cas.Case_Id AND RowNum=2 AND tep.CPT_ATUO_ID = cod.CPT_ATUO_ID)as TT_PreC_2,	
		(SELECT convert(varchar(10), VDate,120) FROM #TempVol tep (NOLOCK) WHERE tep.VCaseId=cas.Case_Id AND RowNum=2 AND tep.CPT_ATUO_ID = cod.CPT_ATUO_ID)as TD_PreC_2,
		(SELECT VPay FROM #TempVol tep (NOLOCK) WHERE tep.VCaseId=cas.Case_Id AND RowNum=3 AND tep.CPT_ATUO_ID = cod.CPT_ATUO_ID)as TT_PreC_3,	
		(SELECT convert(varchar(10), VDate,120) FROM #TempVol tep (NOLOCK) WHERE tep.VCaseId=cas.Case_Id AND RowNum=3 AND tep.CPT_ATUO_ID = cod.CPT_ATUO_ID) AS TD_PreC_3,
		--VDedct AS Deductible,
		--VIntr  as Interest,
		ISNULL((SELECT SUM(ISNULL(VDedct,0.00)) FROM #TempVol tep (NOLOCK) WHERE tep.VCaseId=cas.Case_Id AND tep.CPT_ATUO_ID = cod.CPT_ATUO_ID), 0.00) as Deductible,
	    ISNULL((SELECT SUM(ISNULL(VIntr,0.00)) FROM #TempVol tep (NOLOCK) WHERE tep.VCaseId=cas.Case_Id AND tep.CPT_ATUO_ID = cod.CPT_ATUO_ID), 0.00) as Interest,
		...
		((CAST(isnull((Select SUM(ISNULL(BCOD.Amount,0)) from tbltreatment Trtmnt INNER JOIN BILLS_WITH_PROCEDURE_CODES BCOD ON BCOD.fk_Treatment_Id = Trtmnt.Treatment_Id where Trtmnt.case_id=Tre.case_id ),0)AS FLOAT)
		- CAST(ISNULL(cod.Bill_Adjustment,0) AS FLOAT) - CAST(ISNULL((SELECT SUM(ISNULL(VPay,0.00)) FROM #TempVol tep (NOLOCK) WHERE tep.VCaseId=cas.Case_Id AND tep.CPT_ATUO_ID = cod.CPT_ATUO_ID), 0.00) AS FLOAT) 
		- CAST(ISNULL((SELECT SUM(ISNULL(VDedct,0.00)) FROM #TempVol tep (NOLOCK) WHERE tep.VCaseId=cas.Case_Id AND tep.CPT_ATUO_ID = cod.CPT_ATUO_ID), 0.00) AS FLOAT) + CAST(ISNULL((SELECT SUM(ISNULL(VIntr,0.00)) FROM #TempVol tep (NOLOCK) WHERE tep.VCaseId=cas.Case_Id AND tep.CPT_ATUO_ID = cod.CPT_ATUO_ID), 0.00) AS FLOAT))
