...
CREATE PROCEDURE [dbo].[XN_TEMP_INSERT_CASE_DETAILS_GBB_Dino]
...
BEGIN
	...
	DECLARE @s_l_gbb_type varchar(50)
	
	----- set max id here and change table name-----
	SET @autoidtblcase = 510979 --(select max(case_autoid) from tblcase Where  [DomainId] = 'dl') -- 510796
	Set @s_a_DomainId = 'DL'
	SET @s_l_gbb_type = 'GYB'
	...
	FROM [XN_TEMP_TABLE_GBB_DATA_GYB_ALL_Dion] where ISNULL(insurancecompanyid ,'') <> ''  and ISNULL(Status_Done,'') = ''  and ISNULL(provider_id,'') <> ''
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN			
				...
				SET
					...
					DenialReason_ID = (SELECT top 1 DenialReasons_Id from tblDenialReasons WHERE DENIALREASONS_TYPE = @Denial_Reason1 AND DomainId = @s_a_DomainId),
				...
				AND DenialReasons_Id = (SELECT top 1 DenialReasons_Id from tblDenialReasons WHERE DENIALREASONS_TYPE = @Denial_Reason2 AND DomainId = @s_a_DomainId))
				BEGIN
					INSERT TXN_tblTreatment (Treatment_Id,DenialReasons_Id, DomainID)
					Select Treatment_Id,(select TOP 1 DenialReasons_Id from tblDenialReasons where DENIALREASONS_TYPE = @Denial_Reason2 and DENIALREASONS_TYPE <> '') as DenialReasons_Id , @s_a_DomainId from TBLTREATMENT WHERE CASE_ID = @Case_Id and BILL_NUMBER = @SZ_BILL_NUMBER And [DomainId] = @s_a_DomainId
				END
				
				
				IF NOT EXISTS(SELECT * FROM TXN_tblTreatment WHERE Treatment_Id = (select Treatment_Id from tblTreatment WHERE CASE_ID = @Case_Id and BILL_NUMBER = @SZ_BILL_NUMBER And [DomainId] = @s_a_DomainId)
				AND DenialReasons_Id = (SELECT top 1 DenialReasons_Id from tblDenialReasons WHERE DENIALREASONS_TYPE = @Denial_Reason3 AND DomainId = @s_a_DomainId))
				BEGIN
					INSERT TXN_tblTreatment (Treatment_Id,DenialReasons_Id, DomainId)
					Select Treatment_Id,(select top 1 DenialReasons_Id from tblDenialReasons where DENIALREASONS_TYPE = @Denial_Reason3 and DENIALREASONS_TYPE <> '') as DenialReasons_Id, @s_a_DomainId  from TBLTREATMENT WHERE CASE_ID = @Case_Id and BILL_NUMBER = @SZ_BILL_NUMBER 
				END
	  ...
	  UPDATE [XN_TEMP_TABLE_GBB_DATA_GYB_ALL_Dion]
