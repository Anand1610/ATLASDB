...
CREATE PROCEDURE [dbo].[Peer_Review_Doctor_Add]
(
   @i_a_Doctor_id			INT,
...
BEGIN
	...
	IF(@i_a_Doctor_id = 0)
	BEGIN
	    IF EXISTS(SELECT Doctor_Name FROM TblReviewingDoctor WHERE Doctor_Name = @s_a_Doctor_Name and DomainID = @s_a_DomainID)
	    BEGIN
	       SET @s_l_message		=  'Doctor name already exists..!!!'
		   SET @i_l_result		=  SCOPE_IDENTITY()
	    END
	    ELSE
	    BEGIN
	          BEGIN TRAN
		      INSERT INTO TblReviewingDoctor
		      (
			      Doctor_Name,
				  DomainID,	
				  Active,						
	...
	BEGIN
		...
		SET @oldDoctor_Name = (SELECT TOP 1 Doctor_Name FROM TblReviewingDoctor WHERE  Doctor_id = @i_a_Doctor_id and DomainID = @s_a_DomainID )
		
		IF NOT EXISTS(SELECT TOP 1 Doctor_Name FROM TblReviewingDoctor WHERE  Doctor_id <>  @i_a_Doctor_id and Doctor_Name= @s_a_Doctor_Name and DomainID = @s_a_DomainID )
		BEGIN
			UPDATE TblReviewingDoctor
			SET 
