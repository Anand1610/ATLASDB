CREATE PROCEDURE [dbo].[PortfolioSettlementForClient]     
(    
 @PortfolioId INT ,    
 @DomainId VARCHAR(50),    
...
SELECT tc.Case_Id,    
    tc.Claim_Amount,    
    tc.InjuredParty_FirstName +' '+tc.InjuredParty_LastName as InsuredName,  
...
FROM  tblcase tc     
	...
	WHERE  tc.[DomainId] =@DomainId  AND tpf.Id=@PortfolioId AND ISNULL(tc.IsDeleted,0) = 0   
	...
	WHILE(@CounterInsert <= @TotalCountInsert)    
		BEGIN    
		 ...
		 SELECT  @FAMT = Funding_Amt,    
			@IFEE = IFee,    
			@MFEE = MFee,    
			@CLAIM_AMT = CASE WHEN Claim_Amt IS NULL THEN 0 ELSE CONVERT(NUMERIC(18,2),Claim_Amt) END,    
...
SELECT SUM(CONVERT(NUMERIC(18,2),Claim_Amt)) AS Total_Claim,    
	   ...
	   (select SUM(CONVERT(NUMERIC(18,2),pp.Claim_Amt)) from @PortfolioPaymentSettlement pp where Settled=1)  AS Total_Claim_Settled,  
	   (select SUM(CONVERT(NUMERIC(18,2),pp.Settled_amt)) from @PortfolioPaymentSettlement pp where Settled=1)  AS Total_Settled_Amt_Settled,  
	   (select SUM(CONVERT(NUMERIC(18,2),pp.Expected_payment)) from @PortfolioPaymentSettlement pp where Settled=1)  AS Expected_Payment_Settled,  
	   (select SUM(CONVERT(NUMERIC(18,2),pp.Client_payment)) from @PortfolioPaymentSettlement pp where Settled=1)  AS Client_Payment_Settled,  
	   (select SUM(CONVERT(NUMERIC(18,2),pp.Claim_Amt)) from @PortfolioPaymentSettlement pp where Settled=0)  AS Total_Claim_Unsettled,  
	   (select SUM(CONVERT(NUMERIC(18,2),pp.Settled_amt)) from @PortfolioPaymentSettlement pp where Settled=0)  AS Total_Settled_Amt_Unsettled,  
	   (select SUM(CONVERT(NUMERIC(18,2),pp.Expected_payment)) from @PortfolioPaymentSettlement pp where Settled=0)  AS Expected_Payment_Unsettled,  
	   (select SUM(CONVERT(NUMERIC(18,2),pp.Client_payment)) from @PortfolioPaymentSettlement pp where Settled=0)  AS Client_Payment_Unsettled,  
	   IFee,  
	   MFee,  
	   Reserved,  
	   Fixed_Fee_rate_Time,  
