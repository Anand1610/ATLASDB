...
CREATE PROCEDURE [dbo].[XN_TEMP_INSERT_CASE_DETAILS_GBB_mccollum]
...
BEGIN
	...
	SET @autoidtblcase = 0 --(select max(case_autoid) from tblcase Where  [DomainId] = 'mccollum') -- 14711
	Set @s_a_DomainId = 'mccollum'
	...
	FROM [XN_TEMP_TABLE_DATA_MCCOLLUM] where ISNULL(insurancecompanyid ,'') <> ''  and ISNULL(Status_Done,'') = ''  and ISNULL(provider_id,'') <> '' And [DomainId] = @s_a_DomainId
	...
	BEGIN
	  ...
	  UPDATE [XN_TEMP_TABLE_DATA_MCCOLLUM]
