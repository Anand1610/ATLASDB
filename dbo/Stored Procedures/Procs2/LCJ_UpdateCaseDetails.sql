...
-----[LCJ_UpdateCaseDetails] 'JL' ,'JL20-114336','','DOCUMENTS MISSING','Verification_Status','admin','','Documents Missing'
	 ...
	 IF(@DomainId = 'jl' AND @fieldName = 'Verification_Status')
		...
		IF(@newValue <> @oldTextName)
			BEGIN
				...
				UPDATE tblCase_additional_info SET VerificationStatus = @newValue where case_id = @Case_Id and domainid=@DomainId
		...
		BEGIN		
			IF(@newValue <> @oldTextName)
				...
				DECLARE @NoteDesc varchar(1000)
					UPDATE tblCase_additional_info SET VerificationDate = @newValue where case_id = @Case_Id and domainid=@DomainId
						IF(@oldTextName <> '')
							BEGIN
